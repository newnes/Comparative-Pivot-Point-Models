...
@click.command()
@click.option("--csv_file", "-c", type=str, default="data/ALL.csv", help="CSV file for analysis (default: data/ALL.csv)")
@click.option("--fig_name", "-f", type=str, default="metrics", help="name for the png image metric results (default: metrics)")
def main(csv_file, fig_name):
    ...
    all_weeks = pd.read_csv(csv_file, low_memory=False)
    ...
    for i in tqdm(range(1, len(all_days)), desc="Searching..."):
    ...
    plt.savefig(f"output/{fig_name}.svg", bbox_inches="tight")
